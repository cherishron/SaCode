...
此目录用于存放用户自定义的钩子脚本，支持在关键操作前后执行自定义逻辑。
...
```typescript
// 简单函数导出
import type { HookContext, HookResult } from "@saclaw/core";
...
```typescript
import type { HookDefinition } from "@saclaw/core";
...
```typescript
// hooks/pre-command/block-dangerous.ts
