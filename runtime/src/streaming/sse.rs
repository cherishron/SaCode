...
use crate::daemon::{DaemonMetrics, EventHistory, StreamEvent};

pub type SseResponse = Sse<Pin<Box<dyn Stream<Item = Result<Event, Infallible>> + Send>>>;

/// 连接生命周期守卫：构造时记 opened/active，Drop 时记 closed 与连接时长。
struct SseSession {
...
impl SseSession {
    fn open(metrics: Arc<DaemonMetrics>) -> Self {
        metrics.sse.opened.fetch_add(1, Ordering::Relaxed);
        metrics.sse.active.fetch_add(1, Ordering::Relaxed);
        Self {
            metrics,
...
impl Drop for SseSession {
    fn drop(&mut self) {
        self.metrics.sse.active.fetch_sub(1, Ordering::Relaxed);
        self.metrics.sse.closed.fetch_add(1, Ordering::Relaxed);
...
{
    let session = SseSession::open(metrics.clone());
    inner.map(move |evt| {
        let _keep_alive = &session;
        metrics.sse.delivered.fetch_add(1, Ordering::Relaxed);
        evt
    })
}
...
fn build_sse_response<S>(stream: S, metrics: Arc<DaemonMetrics>) -> SseResponse
where
    S: Stream<Item = StreamEvent> + Send + 'static,
{
    let event_stream = attach_sse_metrics(stream, metrics).map(|evt| {
...
pub fn stream_from_broadcast(
    ...
    build_sse_response(
        filter_stream_events(receiver, task_filter, Some(metrics.clone())),
        metrics,
    )
}
    ...
    if last_event_id.is_some() {
        if let Some(metrics) = metrics.as_ref() {
            ...
            metrics
                .sse
                .replayed_events
                .fetch_add(replay_filtered.len() as u64, Ordering::Relaxed);
        }
    }

    // 3. replay 流 + live 流（去重）
    let replay_stream = stream::iter(replay_filtered);
    let live_stream = filter_stream_events(receiver, task_filter, metrics).filter(move |evt| {
        ...
        loop {
            match receiver.recv().await {
                ...
                Err(broadcast::error::RecvError::Lagged(skipped)) => {
                    ...
                    if let Some(metrics) = metrics.as_ref() {
                        metrics.sse.lagged.fetch_add(1, Ordering::Relaxed);
                        metrics.sse.skipped.fetch_add(skipped, Ordering::Relaxed);
                    }
                    yield StreamEvent {
                        task_id: String::new(),
...
mod tests {
    ...
    async fn closes_after_terminal_event_with_task_filter() {
        ...
        let stream = filter_stream_events(rx, Some("task-1".to_string()), None);
    ...
    async fn emits_lagged_hint_on_overflow() {
        ...
        let stream = filter_stream_events(rx, None, None);
    ...
    async fn global_stream_does_not_close_on_terminal_event() {
        let (tx, rx) = broadcast::channel(16);
        let mut stream = Box::pin(filter_stream_events(rx, None, None));
    ...
    async fn replay_then_live_without_duplicate() {
        ...
        let stream =
            replay_then_live_stream(rx, Some("task-1".to_string()), &history, Some(0), None);
    ...
    #[tokio::test]
    async fn sse_metrics_track_connection_lifecycle_and_delivery() {
        let metrics = Arc::new(DaemonMetrics::default());
        let (tx, rx) = broadcast::channel(16);
        let stream = attach_sse_metrics(
            filter_stream_events(rx, None, Some(metrics.clone())),
            metrics.clone(),
        );
        assert_eq!(metrics.sse.opened.load(Ordering::Relaxed), 1);
        assert_eq!(metrics.sse.active.load(Ordering::Relaxed), 1);

        tx.send(make_event("task-1", "task_started")).unwrap();
        drop(tx);

        let types = collect_event_types(stream).await;
        assert_eq!(types, vec!["task_started"]);
        assert_eq!(metrics.sse.delivered.load(Ordering::Relaxed), 1);
        assert_eq!(metrics.sse.active.load(Ordering::Relaxed), 0);
        assert_eq!(metrics.sse.closed.load(Ordering::Relaxed), 1);
    }

    #[tokio::test]
    async fn sse_metrics_count_lagged_and_skipped() {
        let metrics = Arc::new(DaemonMetrics::default());
        let (tx, rx) = broadcast::channel(2);
        let stream = filter_stream_events(rx, None, Some(metrics.clone()));
        ...
        let types = collect_event_types(stream).await;
        assert!(types.iter().any(|t| t == "lagged"));
        assert!(metrics.sse.lagged.load(Ordering::Relaxed) >= 1);
        assert!(metrics.sse.skipped.load(Ordering::Relaxed) >= 1);
    }

    #[tokio::test]
    async fn sse_metrics_count_replay_connections_and_events() {
        let metrics = Arc::new(DaemonMetrics::default());
        let (tx, rx) = broadcast::channel(16);
        let history = EventHistory::new(32);
        ...
        let mut live_evt = make_event("task-1", "task_completed");
        history.push(&mut live_evt);
        tx.send(live_evt).unwrap();
        drop(tx);

        let types = collect_event_types(stream).await;
        assert_eq!(types, vec!["task_started", "msg-1", "task_completed"]);
        assert_eq!(metrics.sse.replay_connections.load(Ordering::Relaxed), 1);
        assert_eq!(metrics.sse.replayed_events.load(Ordering::Relaxed), 2);
    }
}
