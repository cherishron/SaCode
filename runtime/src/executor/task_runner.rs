...
use crate::provider::{ProviderClient, StreamChunk, StreamChunkKind, ToolChatResult};
use crate::tools::{SideEffectLevel, ToolRegistry};
use crate::{task_run_snapshot, FailoverContext, NodeScore, TaskProfile};
...
pub async fn execute_task_with_provider(
    ...
    let task_run = task_run_snapshot(
        ...
        response
            ...
            .ok()
            .cloned()
            .or_else(|| response.as_ref().err().cloned()),
    );
