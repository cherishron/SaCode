...
| 阶段 | 目标 | 周次 | 状态 |
|------|------|------|------|
| P0 体验闭环 | 首次配置 ≤2 步、渐进式文档 | W1-W2 | 部分验收（流程已实现，成功率与恢复指标待验收） |
| P1 VSCode 扩展 MVP | 复用 sacode serve、侧边栏对话 | W3-W5 | 部分验收（基础任务与审批已实现，跨入口一致性待验收） |
| P2 定位聚焦 | PRD/roadmap 修订、平台化收敛 | W6 | 已验收（PRD 与路线图已明确收敛范围） |
| P3 简化过度设计 | /goal 轻量、知识 9→3、冲突简化 | W7-W8 | 部分验收（能力已实现，统一证据待补） |
| P4 强化核心机制 | 三级模式视觉、沙箱审计文档、审批可视化 | W9-W10 | 部分验收（安全异常场景待补） |
| P5 场景补齐 | CI/CD、代码审查、远程开发、测试编写 | W11-W12 | 部分验收（四类端到端验收待补） |

当前状态使用统一词汇，并由 `docs/product/status.json` 作为机器可读真相源：

| 能力 ID | 状态 | 验收依据或缺口 |
|---|---|---|
| p0-first-use | 部分验收 | 首次配置流程已实现，端到端成功率与失败恢复指标待验收 |
| p1-vscode-mvp | 部分验收 | 基础任务和审批已实现，跨入口一致性与选区透明待验收 |
| p2-positioning | 已验收 | PRD 与路线图已明确平台化收敛范围 |
| p3-simplification | 部分验收 | 轻量目标、记忆收敛和审批拦截已实现，统一验收证据待补 |
| p4-core-mechanisms | 部分验收 | 模式、审计和审批能力存在，安全异常场景验收待补 |
| p5-scenarios | 部分验收 | 场景资料和能力存在，四类端到端验收待补 |
...
| `v1.2+` | 体验闭环与简化 | IDE 插件、provider 零配置、首次体验、过度设计简化 | 部分验收（P0-P5 均按统一证据补齐异常与端到端验收） |
