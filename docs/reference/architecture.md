...
灵枢（ Ling Shu ）是 SaCode 的核心技术品牌名称，源自《黄帝内经》经络体系，隐喻智能系统的自组织、自防护、自愈合能力。

### 9.1 架构理念

| 灵枢隐喻 | SaCode 实践 | 核心价值 |
|----------|-------------|----------|
| 经络协调脏腑 | 角色驱动编排 | 多角色协同，各司其职 |
| 诊察经脉病候 | 五维冲突检测 | 多维度识别，主动拦截 |
| 表里经备用通路 | 故障转移路由 | 弹性容错，永不中断 |
...
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐   │
│  │  自组织      │    │  自防护      │    │  自愈合      │   │
│  │  角色驱动编排 │    │  五维冲突检测 │    │  故障转移路由 │   │
│  ├─────────────┤    ├─────────────┤    ├─────────────┤   │
│  │ orchestrator│    │summary_     │    │ model_      │   │
│  │ role_registry│    │compactor    │    │ routing     │   │
│  │ planner     │    │ConflictRecord│    │ Failover    │   │
│  └─────────────┘    └─────────────┘    └─────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
**关键数据结构**：
- `RoleRegistry`：内置角色定义（requirement-analyst、system-architect、implementer 等）
- `Orchestrator`：编排执行器，调用 `execute_role_driven_orchestration`
- `WorkerAgent`：子 agent 执行器，运行单角色任务

**执行流程**：
```
任务输入 → TaskProfile 分析 → 角色选择 → 并行执行子 agent → 结果汇总 → 摘要生成
```

### 9.4 自防护 — 五维冲突检测

**代码位置**：`runtime/src/agents/summary_compactor.rs`、`kernel/src/execution/report.rs`

**职责**：
- 检测多角色输出间的冲突与分歧
- 识别风险信号和负面极性
- 生成结构化冲突记录 `ConflictRecord`

**五维检测维度**（待文档化，当前实现覆盖）：
1. 语义一致性：角色间结论是否矛盾
2. 风险信号：是否包含 fail/error/blocked 等关键词
3. 上下文完整性：是否遗漏关键信息
4. 执行状态：是否出现 regression/broken
5. 建议有效性：推荐行动是否可执行

**关键数据结构**：
- `ConflictRecord`：冲突记录（kind、summary、details）
- `SummaryRecord`：结构化摘要，包含冲突列表
- `detect_output_polarity`：输出极性检测

### 9.5 自愈合 — 故障转移路由

**代码位置**：`runtime/src/model_routing/`、`runtime/src/agents/model_router.rs`
...
**关键数据结构**：
- `TaskProfile`：语言、框架、表面、任务类型、风险级别
- `ModelRoutePlan`：主模型 + 备选模型列表
- `FailoverContext`：故障切换上下文，保留已完成步骤

**故障转移策略**：
1. 主模型执行失败 → 自动切换到评分最高的备选模型
2. 保留已完成步骤和关键事实（retained_facts）
3. 传递失败原因供下一个模型参考

### 9.6 技术定位

| 能力维度 | 传统 Agent 框架 | 灵枢架构 |
|----------|----------------|----------|
| 任务编排 | 单 Agent / 固定流水线 | 角色驱动动态编排 |
| 冲突处理 | 无 / 事后报错 | 五维实时检测与拦截 |
| 模型容错 | 单模型绑定 | 智能路由 + 故障自动转移 |
| 系统韧性 | 依赖外部保障 | 自组织 + 自防护 + 自愈合 |
...
├──────────────┴──────────────┴────────────┤
│              kernel 层                   │
│    ExecutionReport / ConflictRecord     │
├─────────────────────────────────────────┤
│              runtime 层                  │
│   agents / model_routing / memory       │
├─────────────────────────────────────────┤
│           interfaces/cli 层             │
│     CLI / TUI / REPL / MCP Server      │
└─────────────────────────────────────────┘
```
