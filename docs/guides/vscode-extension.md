...
如果客户端完全未应答，daemon 会在 300 秒后自动拒绝，任务不会因缺少 UI 响应而无限等待。
...
当前扩展会报告流错误，但不自动携带 `Last-Event-ID` 重连。可以先查询任务结果/状态；自定义客户端可使用 `/api/stream?task_id=<ID>` 与 `Last-Event-ID` 续传。历史缓冲只有 256 条且 daemon 重启后不保留，不能把它当作持久化事件日志。
