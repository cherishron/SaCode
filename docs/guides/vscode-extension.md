...
`fs.edit` 和 `fs.apply_patch` 审批会优先打开 VSCode 原生 Diff Editor，左侧显示当前工作区内容，右侧显示批准后的候选内容。`fs.apply_patch` 含多个文件时按文件顺序审阅，可接受或拒绝单个文件；部分接受时 daemon 仅应用被接受的路径。预览解析或文件读取失败会显示警告并安全回退到普通 QuickPick。

其他工具继续使用 QuickPick。关闭 QuickPick 等价于拒绝，理由为 `user_dismissed`；显式拒绝使用 `user_denied`。审批提交失败时，扩展允许用户重试或关闭。daemon 端等待超过 300 秒、任务取消或审批通道关闭时都会默认拒绝。
